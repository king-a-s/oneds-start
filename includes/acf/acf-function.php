...
define('ODS_ACF_PATH', plugin_dir_path(__FILE__));
define('ODS_ACF_URL', plugin_dir_url(__FILE__));
...
function ods_disable_create_newpost()
{
...
// add_action('admin_menu', 'ods_disable_create_newpost');
...
/**
 * include acf form head before header for frontend (with short code) example
 */

function ods_include_acf_form_head_before_header_SHORT_CODE()
{
    ...
    if (is_page() && has_shortcode(get_post()->post_content, 'SHORT_CODE')) {
        acf_form_head();
        // echo '<pre>';
        // var_dump('test');
        // echo '</pre>';
...
add_action('wp', 'ods_include_acf_form_head_before_header_SHORT_CODE');
